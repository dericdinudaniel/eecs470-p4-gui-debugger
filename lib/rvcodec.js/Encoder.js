...
export class Encoder {
   ...
   */
    ...
    if (/^s[lr][al]i/.test(this.#mne)) {
      ...
      if (
        this.#config.isa === COPTS_ISA.RV32I ||
        this.#inst.opcode === OPCODE.OP_IMM_32
      ) {
        shamtWidth = FIELDS.i_shamt.pos[1]; // 5bit width (RV32I)
      } else if (
        this.#config.isa === COPTS_ISA.RV64I ||
        this.#inst.opcode === OPCODE.OP_IMM_64
      ) {
   ...
   */
    ...
    if (loadExt) {
      ...
      // Get operands
      const predecessor = this.#opr[0],
        successor = this.#opr[1];

      // Convert to binary representation
      const pred = encMem(predecessor),
   ...
   */
    ...
    // Construct binary instruction
    this.bin =
      imm_11_5 + rs2 + rs1 + this.#inst.funct3 + imm_4_0 + this.#inst.opcode;
  }
   ...
   */
    ...
    const imm_12 = imm.substring(0, len_12),
      ...
      imm_4_1 = imm.substring(
        len_12 + len_11 + len_10_5,
   ...
   */
    ...
    const imm_20 = imm.substring(0, len_20),
      ...
      imm_10_1 = imm.substring(
        len_20 + len_19_12 + len_11,
   ...
   */
    ...
    // Convert to binary representation
    const rd = encReg(dest, true),
      rs1 = encReg(src1, true),
      rs2 = encReg(src2, true),
      rs3 = encReg(src3, true),
      fmt = this.#inst.fp_fmt,
      funct3 = encFrm(frm) ?? "111"; /* dyn rm */

    // Construct binary instruction
    this.bin = rs3 + fmt + rs2 + rs1 + funct3 + rd + this.#inst.opcode;
  }
   ...
   */
    ...
    const rdRs1 =
      this.#inst.rdRs1Val !== undefined
        ? encImm(this.#inst.rdRs1Val, FIELDS.c_rd_rs1.pos[1])
        : destSrc1 === undefined
        ? "01000"
        : encReg(destSrc1);
    const rs2 =
      this.#inst.rs2Val !== undefined
    ...
    if (this.#inst.rdRs1Excl !== undefined) {
      ...
      for (const excl of this.#inst.rdRs1Excl) {
        if (val === excl) {
          throw `Illegal value "${destSrc1}" in rd/rs1 field for instruction ${
            this.#mne
          }`;
        }
      }
    ...
    if (this.#inst.rs2Excl !== undefined) {
      ...
      for (const excl of this.#inst.rs2Excl) {
        if (val === excl) {
          throw `Illegal value "${src2}" in rs2 field for instruction ${
            this.#mne
          }`;
        }
      }
   ...
   */
    ...
    if (this.#inst.nzimm && (immVal === 0 || isNaN(immVal))) {
      ...
      if (immediate === undefined) {
        ...
        throw `Invalid immediate "${immediate}", ${
          this.#mne
        } instruction expects non-zero value`;
      }
    }
    if (this.#inst.uimm && immVal < 0) {
      throw `Invalid immediate "${immediate}", ${
        this.#mne
      } instruction expects non-negative value`;
    }
   ...
   */
    ...
    // Validate operands
    if (this.#inst.uimm && immVal < 0) {
      throw `Invalid immediate "${offset}", ${
        this.#mne
      } instruction expects non-negative value`;
    }
   ...
   */
    ...
    // Construct binary instruction
    this.bin =
      this.#inst.funct3 + imm0 + rs1Prime + imm1 + rdPrime + this.#inst.opcode;
  }
   ...
   */
    ...
    // Construct binary instruction
    this.bin =
      this.#inst.funct3 + imm0 + rs1Prime + imm1 + rs2Prime + this.#inst.opcode;
  }
   ...
   */
    ...
    // Construct binary instruction
    this.bin =
      this.#inst.funct3 + imm0 + funct2 + rdRs1Prime + imm1 + this.#inst.opcode;
  }
