...
export class Decoder {
  ...
  #convertBinToAsm() {
      ...
      switch (fmt) {
        ...
        default:
          throw `Internal error: Detected ${
            this.#mne
          } in quadrant ${quadrant} but could not match instruction format`;
      }
    }
    ...
    if (
      this.#config.ISA === COPTS_ISA.RV32I &&
      /^RV(?:64|128)/.test(this.isa)
    ) {
      throw `Detected ${this.isa} instruction but configuration ISA set to RV32I`;
    } else if (
      this.#config.ISA === COPTS_ISA.RV64I &&
      /^RV128/.test(this.isa)
    ) {
  ...
  #decodeOP_IMM() {
    ...
    if (shift) {
      ...
      if (op_imm_32) {
        ...
        this.#config.ISA === COPTS_ISA.RV32I ||
        (this.#config.ISA === COPTS_ISA.AUTO &&
          shamt_6 === "0" &&
          shamt_5 === "0")
      ) {
        shamtWidth = 5;
      ...
      if (shamtWidth === 7) {
        ...
        f["imm"] = new Frag(
          ...
          shamt,
          shamt_6_0,
          FIELDS.i_shamt_6_0.name
        );
        f["shift"] = new Frag(
          FRAG.OPC,
          this.#mne,
          imm_11_7,
          FIELDS.i_shtyp_11_7.name
        );
        ...
        f["imm"] = new Frag(
          ...
          shamt,
          shamt_5_0,
          FIELDS.i_shamt_5_0.name
        );
        f["shift"] = new Frag(
          FRAG.OPC,
          this.#mne,
          imm_11_6,
          FIELDS.i_shtyp_11_6.name
        );
      ...
      if (
        (shamtWidth === 5 && imm_11_5 !== imm.substring(0, 7)) ||
  ...
  #decodeSYSTEM() {
    ...
    if (trap) {
      ...
      // Binary fragments from MSB to LSB
      this.binFrags.push(f["csr"], f["rs1"], f["funct3"], f["rd"], f["opcode"]);
    }
  }
  ...
  #decodeUType() {
    ...
    // Convert fields to string representations
    const immediate = decImm(imm_31_12),
      dest = decReg(rd);
  ...
  #decodeCR(inst) {
    ...
    if (inst.rdRs1Excl?.includes(destSrc1Val)) {
      throw `Detected ${
        this.#mne
      } instruction, but illegal value "${destSrc1}" in rd/rs1 field`;
    }
    const src2Val = decImm(rs2, false);
    if (inst.rs2Excl?.includes(src2Val)) {
      throw `Detected ${
        this.#mne
      } instruction, but illegal value "${src2}" in rs2 field`;
    }
  ...
  #decodeCI(inst) {
    ...
    if (shiftInst) {
      if (immVal === 0) {
        ...
        inst = ISA[this.#mne];
        if (inst === undefined) {
          throw `Internal error when converting shift-immediate instruction into ${
            this.#mne
          }`;
        }
        // Overwrite ISA
    ...
    if (inst.nzimm && immVal === 0) {
      throw `Detected ${
        this.#mne
      }, but instruction expects non-zero immediate value (encoding reserved)`;
    }
    ...
    immName += shiftInst ? FIELDS.c_shamt_0.name : FIELDS.c_imm_ci_0.name;
    ...
    if (dynamicImm) {
      f["imm0"] = new Frag(
        ...
        immVal,
        imm0,
        immName + immBitsToString(immBitsLabels[0])
      );
      f["imm1"] = new Frag(
        FRAG.IMM,
        immVal,
        imm1,
        immName + immBitsToString(immBitsLabels[1])
      );
    } else {
      f["imm0"] = new Frag(
        FRAG.OPC,
        this.#mne,
        imm0,
        "static-" + immName + immBitsToString(immBitsLabels[0])
      );
      f["imm1"] = new Frag(
        FRAG.OPC,
        this.#mne,
        imm1,
        "static-" + immName + immBitsToString(immBitsLabels[1])
      );
    }
  ...
  #decodeCSS(inst) {
    ...
    const f = {
      ...
      imm: new Frag(
        ...
        offset,
        imm,
        immName + immBitsToString(inst.immBits)
      ),
    };
  ...
  #decodeCL(inst) {
    ...
    const f = {
      ...
      imm0: new Frag(
        ...
        offset,
        imm0,
        immName + immBitsToString(inst.immBits[0])
      ),
      imm1: new Frag(
        FRAG.IMM,
        offset,
        imm1,
        immName + immBitsToString(inst.immBits[1])
      ),
    };
  ...
  #decodeCA() {
    ...
    const f = {
      ...
      rd_rs1_prime: new Frag(
        FRAG.RD,
        destSrc1,
        rdRs1Prime,
        FIELDS.c_rs2_prime.name
      ),
    ...
    if (branchInst) {
      ...
      if (dynamicImm) {
        ...
        f["imm0"] = new Frag(
          ...
          this.#mne,
          shamt0,
          "static-" + immName + immBitsToString(inst.immBits[0])
        );
        f["imm1"] = new Frag(
          FRAG.OPC,
          this.#mne,
          shamt1,
          "static-" + immName + immBitsToString(inst.immBits[1])
        );
      }
  ...
  #decodeCJ(inst) {
    ...
    const f = {
      ...
      imm: new Frag(
        ...
        jumpTarget,
        imm,
        FIELDS.c_imm_cj.name + immBitsToString(inst.immBits)
      ),
    };
...
function decImm(immediate, signExtend = true) {
  ...
  if (signExtend && immediate[0] === "1") {
    return (
      parseInt(immediate, BASE.bin) -
      Number("0b1" + "".padStart(immediate.length, "0"))
    );
  }
...
export function decRegAbi(regDec, floatReg = false) {
  return Object.keys(floatReg ? FLOAT_REGISTER : REGISTER)[
...
function decCSR(binStr) {
  ...
  let csr = entry ? entry[0] : "0x" + val.toString(16).padStart(3, "0");
...
function regExclToString(excl) {
  if (excl.length === 1) return excl[0].toString();
  let out = "{";
  let addComma = false;
  for (const e of excl) {
    if (!addComma) {
...
function renderAsm(asmFrags, abi = false) {
  ...
  for (let i = 1; i < asmFrags.length; i++) {
    ...
    if (i === 1) {
      ...
      !asmFrags[i].mem ||
      !/^(?:nz)?(?:u)?imm/.test(asmFrags[i - 1].field)
    ) {
      inst += ", ";
    }
